...
void CRadCaliDlg::Process()
{
    ...
    if ( m_bAdj ){
        ...
        ProgBegin(sum*4);
        for (c = 0; c < 4; c++) {
            const int nUnknown = sum * 4;
            const int maxIter = 2; // 0: 初解, 1: 鲁棒重权
            std::vector<double> lastX; lastX.resize(nUnknown, 0.0);

            for (int iter = 0; iter < maxIter; ++iter) {
                UINT st = GetTickCount();
                print2Log("proc band %d iter %d, Norml ... st=%u\n", c + 1, iter + 1, st);

                memset(aa, 0, sizeof(double) * nUnknown * nUnknown);
                memset(b, 0, sizeof(double) * nUnknown);
                bool haveResidualBase = (iter > 0); // 第二次需要 residual 预测
                ...
                for (i = 0; i < sum; i++, ProgStep(cancel)) {
                    ...
                    while (!feof(fTsk)) {
                        ...
                        if (olpF.Load4File(strOlp)) {
                            ...
                            int v, oz; OBV* pOs = olpF.GetData(&oz);
                            for (v = 0; v < oz; v++, pOs++) {
                                if (idxr == -1) {
                                    ...
                                    memset(a, 0, sizeof(double) * nUnknown);
                                    a[idx * 4 + 0] = -1; a[idx * 4 + 1] = -k1; a[idx * 4 + 2] = -k2; a[idx * 4 + 3] = pOs->cv[c];

                                    double w = 1.0;
                                    if (haveResidualBase) {
                                        // 预测值
                                        double pred = -(lastX[idx * 4 + 0] + k1 * lastX[idx * 4 + 1] + k2 * lastX[idx * 4 + 2] - pOs->cv[c] * lastX[idx * 4 + 3]);
                                        double res = l - pred;
                                        // Huber
                                        double s = 5.0; // scale 可用前一轮残差 RMS 代替
                                        double r = fabs(res) / s;
                                        if (r > 1.0) w = 1.0 / r;
                                        if (r > 6.0) continue; // 极端剔除
                                    }
                                    Nrml(a, nUnknown, l, aa, b, w);
                                }
                                else {
                                    k1 = getKval(1, pOs->csz, pOs->cvz, pOs->cas) - pAK1[idx];
                                    k2 = getKval(4, pOs->csz, pOs->cvz, pOs->cas) - pAK2[idx];
                                    ...
                                    double w = 0.1;
                                    if (haveResidualBase) {
                                        double pred = (lastX[idx * 4 + 0] + k1 * lastX[idx * 4 + 1] + k2 * lastX[idx * 4 + 2]
                                            - lastX[idxr * 4 + 0] - k1r * lastX[idxr * 4 + 1] - k2r * lastX[idxr * 4 + 2]);
                                        double res = l - pred;
                                        double s = 5.0;
                                        double r = fabs(res) / s;
                                        if (r > 1.0) w = 0.1 / r;
                ...
                memset(x, 0, sizeof(double) * nUnknown);
                Solve(aa, b, x, nUnknown, nUnknown);
                print2Log("band %d iter %d solve done.\n", c + 1, iter + 1);

                if (iter == 0) {
                    // 保存供下一轮预测
                    for (int ii = 0; ii < nUnknown; ++ii) lastX[ii] = x[ii];
                }
                else {
                    // 最终写文件
...
static inline double NormDiffMetric(const short c[4], const short r[4]) {
    double s = 0; int n = 0;
    for (int i = 0; i < 4; i++) {
        if (c[i] > 0 && r[i] > 0) {
            double d = fabs(double(c[i]) - r[i]) / (c[i] + r[i] + 1.0);
            s += d; n++;
        }
    }
    return n ? s / n : 1.0;
}
static inline bool BandRatioOk(const short c[4], const short r[4], double minR = 0.25, double maxR = 4.0) {
    for (int i = 0; i < 4; i++) {
        if (c[i] > 0 && r[i] > 0) {
            double ratio = (double)c[i] / (double)r[i];
...
BOOL MchTie(LPCSTR lpstrPar)
{
    ...
    int idx,yy,mm,dd,ho,mi,se,yy1,mm1,dd1,ho1,mi1,se1,utmZn=0,gs=21,ws=30,bTxt=0;
    ...
    while( !feof(fTsk) ){
        ...
        for( r=1;r<rows-gs;r+=gs ){
            for(  c=1;c<cols-gs;c+=gs ){
                ...
                if (varC < 30 || varR < 30) continue; // 低纹理点剔除 (阈值可调)

                double diffNorm = NormDiffMetric(cv, rv);
                if (diffNorm > 0.45) continue; // 归一化平均亮度差 (可调)

                if (!BandRatioOk(cv, rv, 0.3, 3.2)) continue; // 波段比例过滤 (可调)
